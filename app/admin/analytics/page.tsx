        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h1 className="text-3xl md:text-4xl font-black text-primary tracking-tighter flex items-center gap-3">
                    <BarChart3 className="w-7 h-7 text-[#46AEE8]" /> Analytics
                </h1>
                <div className="flex gap-1 bg-white border border-primary/10 rounded-xl p-1">
                    {RANGES.map((r) => (
                        <button
                            key={r}
                            onClick={() => setDays(r)}
                            className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest ${days === r ? 'bg-[#46AEE8] text-white' : 'text-gray-400'}`}
                        >
                            {r}d
                        </button>
                    ))}
            ...
            {isLoading || !analytics ? (
                ...
                <>
                    ...
                    <div className="p-6 bg-white border border-primary/5 rounded-2xl">
                        <h2 className="font-black text-primary uppercase tracking-tight mb-4">Revenue trend</h2>
                    ...
                    <div className="grid lg:grid-cols-2 gap-4">
                        {/* Top products */}
                        <div className="p-6 bg-white border border-primary/5 rounded-2xl">
                            <h2 className="font-black text-primary uppercase tracking-tight mb-4">Top products</h2>
                                    ...
                                    {analytics.topProducts.map((p) => (
                                        <div key={p.name}>
                                            <div className="flex justify-between text-sm mb-1">
                                                <span className="font-semibold text-primary truncate pr-2">{p.name}</span>
                                            ...
                                            <div className="h-2 rounded-full bg-primary/5 overflow-hidden">
                                                <div className="h-full rounded-full bg-[#46AEE8]" style={{ width: `${(p.revenue / topMax) * 100}%` }} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {/* Status breakdown */}
                        <div className="p-6 bg-white border border-primary/5 rounded-2xl">
                            <h2 className="font-black text-primary uppercase tracking-tight mb-4">Orders by status (all time)</h2>
                            <div className="space-y-3">
                                {analytics.statusBreakdown.map((s) => (
                                    <div key={s.status}>
                                        <div className="flex justify-between text-sm mb-1">
                                            <span className="font-semibold text-primary">{s.status}</span>
